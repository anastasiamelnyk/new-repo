...
const Tabs = ({
    ...
    const tabItemClasses = (tabItem) => cx.current({
        'tab-item': true,
        'tab-item-selected': getTabSelected(tabItem)
    });
    ...
    const renderOptions = () => {
        ...
        return options.map(option => (
            <li key={getOptionText(option)} className={tabItemClasses(option)}>
                <button className={classes['tab-button']} onClick={() => setValue(option)}>
                    {getOptionText(option)}
                </button>
                {hasDeleteBtn && <button onClick={() => deleteTab(option)} className={classes['delete-button']}>
